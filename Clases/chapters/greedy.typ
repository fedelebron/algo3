...
#let t = [Hay varias maneras de probar la correctitud de los algoritmos greedy. En general, van a tener mucha flexibilidad para probar este tipo de algoritmos correctos. Tres formas clásicas son:
  - Argumentos de intercambio, donde tomamos una solución óptima mejor y lo más parecida posible a la nuestra, y hacemos un intercambio en esa solución que la hace o mejor que lo que era, o más parecida a la nuestra.
...
#sol[
  ...
  #demo[
    ...
    + Caso base, $t = 1$. Como empezamos con el tanque vacío en $x_1 = 0$, y $M > 0$, el auto tiene que moverse y no tiene combustible al empezar. Luego toda solución debe cargar en la primera estación, y $g_1 = o_1 = 1$. Luego vale $P(1)$.
    + Paso inductivo. Sabemos que vale $P(t)$, queremos demostrar $P(t+1)$. Si $t = h$, entonces $P(t+1)$ es trivial, pues es una implicación con antecedente falso. Luego basta considerar $t < h$, y por lo tanto $t + 1 lt.eq h$. Sabemos por $P(t)$ que $o_t lt.eq g_t$. Sea $A_t = {j | j > g_t, x_j - x_(g_t) lt.eq C}$ el conjunto de estaciones alcanzables yendo hacia adelante desde $g_t$. Como el problema tiene solución, $A_t eq.not emptyset$, y podemos llegar al menos a la próxima estación. Sea entonces $j^* = max A_t$. Nuestro algoritmo elige $g_(t+1) = j^*$.
  ...
  #demo[
    Al igual que antes, vamos a dar por demostrado (porque lo hicimos en la primera demostración) que el algoritmo es correcto cuando no hay solución. Luego vamos a asumir acá que hay solución.
    ...
    - Si el invariante vale antes de una iteración, y la guarda es verdadera, entonces el invariante vale después de la iteración. Asumimos que valen $I(i) and B(i)$. Vamos a probar $I(j)$. Notemos que $j$ es el valor que obtiene $i$ al terminar cada iteración. Tenemos un pequeño ciclo interior que calcula $j = max { l | i lt.eq l < n, x_(l+1) - x_(i + 1) lt.eq C}$ (recordemos que los índices de $x$ empiezan en $1$, mientras que en Python estamos indexando ```python estaciones``` a partir de ```python 0```, por eso los $+1$).
      + Por cómo definimos $j$, tenemos que $j gt.eq i$, y luego $j gt.eq i gt.eq 0$. Asimismo definimos $j$ como el máximo de un conjunto cuyo elementos más grandde es a lo sumo $n - 1$, luego $j lt.eq n - 1$, es decir $j < n$. Luego $0 lt.eq j < n$, que es la primera parte de $I(j)$.
...
#sol[
  ...
  #demo[
    ...
    Supongamos que $A$ no es óptima. Entonces $sum_(i = 1)^n v_i x_i < sum_(i = 1)^n v_i y_i$. Sea $i$ el primer índice donde $x_i eq.not y_i$. Luego, $sum_(j = 1)^(i - 1) x_j p_j = sum_(j = 1)^(i - 1) y_j p_j$. Al momento de elegir $x_i$, nuestro algoritmo podía usar un peso máximo de $P' = P - sum_(j = 1)^(i - 1) x_j p_j$. Nuestro algoritmo elige $x_i = P' / p_i$.
